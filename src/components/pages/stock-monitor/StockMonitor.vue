<template>
  <div class="fixed top-16 left-0 w-full h-[calc(100vh-64px)] bg-slate-50 flex flex-col overflow-hidden">
    ...
    <div class="w-full bg-slate-50 pt-4 pb-3 px-6 z-50 shrink-0 shadow-sm border-b border-slate-100">
      ...
      <div v-else class="flex flex-col gap-2.5">
        ...
        >
          <div class="flex items-center gap-4 min-w-0">
            ...
            <div class="min-w-0">
              <div 
                class="text-[12px] font-black text-slate-800 uppercase leading-tight truncate mb-1"
                v-html="highlightText(p.name, cart.searchQuery)"
              ></div>

              <div class="flex items-center gap-2">
                <span 
                  class="text-[8px] text-slate-400 font-black uppercase bg-slate-50 px-2 py-0.5 rounded-md border border-slate-100"
                  v-html="highlightText(p.category || 'Tanpa Kategori', cart.searchQuery)"
                ></span>
        ...
        <div v-if="filteredProducts.length === 0" class="py-24 text-center opacity-30 flex flex-col items-center">
          ...
          <p class="text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">
            {{ cart.searchQuery ? 'Produk tidak ditemukan' : 'Data Kosong' }}
          </p>
        </div>
      </div>
    </div>
...
// IMPORT LOGIKA DARI UTILS.TXT
import { highlightText } from '@/utils/formatters'; // Sesuaikan path utils Anda
...
const loadStock = async () => {
  ...
  try {
    ...
    console.error("Gagal muat stok:", err);
  } finally {
...
const filteredProducts = computed(() => {
  ...
  if (cart.searchQuery) {
    const q = cart.searchQuery.toLowerCase();
    list = list.filter(p => 
  ...
  if (filterStatus.value === 'kritis') {
    return list.filter(p => (p.stock || p.qty) <= 12);
  }
  if (filterStatus.value === 'menipis') {
    return list.filter(p => (p.stock || p.qty) > 12 && (p.stock || p.qty) <= 36);
  }
...
/* Styling untuk teks yang di-highlight (Logika dari utils.txt) */
:deep(.highlight) {
  background-color: #fef08a; /* Kuning muda */
  color: #854d0e; /* Cokelat tua */
  padding: 0 2px;
  border-radius: 4px;
  font-weight: bold;
}
